...
export function CardsPage() {
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <CardFormModal open={modalOpen} onClose={() => setModalOpen(false)} editing={editing} userId={user?.id ?? ''} ownerName={ownerName} onSaved={() => { setModalOpen(false); refresh(); }} />
...
function CardFormModal({ open, onClose, editing, userId, ownerName, onSaved }: { open: boolean; onClose: () => void; editing: CardType | null; userId: string; ownerName: string; onSaved: () => void }) {
  ...
  const handleSave = async () => {
    ...
    else await supabase.from('cards').insert({ ...payload, user_id: userId, owner_name: ownerName });
    setSaving(false);
    onSaved();
  };
