...
export function GoalsPage() {
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <GoalFormModal open={modalOpen} onClose={() => setModalOpen(false)} editing={editing} userId={user?.id ?? ''} onSaved={() => { setModalOpen(false); refresh(); }} />
...
function GoalFormModal({ open, onClose, editing, userId, onSaved }: { open: boolean; onClose: () => void; editing: Goal | null; userId: string; onSaved: () => void }) {
  ...
  const handleSave = async () => {
    ...
    else await supabase.from('goals').insert({ ...payload, user_id: userId });
    setSaving(false);
    onSaved();
  };
