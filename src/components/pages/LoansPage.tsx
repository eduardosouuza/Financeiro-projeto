...
export function LoansPage() {
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <LoanFormModal open={modalOpen} onClose={() => setModalOpen(false)} editing={editing} userId={user?.id ?? ''} ownerName={ownerName} onSaved={() => { setModalOpen(false); refresh(); }} />
...
function LoanFormModal({ open, onClose, editing, userId, ownerName, onSaved }: { open: boolean; onClose: () => void; editing: Loan | null; userId: string; ownerName: string; onSaved: () => void }) {
  ...
  const handleSave = async () => {
    ...
    else await supabase.from('loans').insert({ ...payload, user_id: userId, owner_name: ownerName });
    setSaving(false);
    onSaved();
  };
