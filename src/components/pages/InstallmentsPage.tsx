...
export function InstallmentsPage() {
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <PurchaseFormModal open={modalOpen} onClose={() => setModalOpen(false)} editing={editing} cards={cards} userId={user?.id ?? ''} onSaved={() => { setModalOpen(false); refresh(); }} />
...
function PurchaseFormModal({ open, onClose, editing, cards, userId, onSaved }: { open: boolean; onClose: () => void; editing: Purchase | null; cards: { id: string; name: string }[]; userId: string; onSaved: () => void }) {
  ...
  const handleSave = async () => {
    ...
    else await supabase.from('purchases').insert({ ...payload, user_id: userId });
    setSaving(false);
    onSaved();
  };
